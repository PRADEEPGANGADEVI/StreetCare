    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-amber-50/70 border border-amber-200 rounded-2xl p-4">
          <div className="text-xs font-bold uppercase text-amber-800 flex items-center gap-1">
            <Award className="w-4 h-4 text-amber-600" /> Bronze Tier
          </div>
        ...
        <div className="bg-slate-100 border border-slate-300 rounded-2xl p-4">
          <div className="text-xs font-bold uppercase text-slate-800 flex items-center gap-1">
            <Award className="w-4 h-4 text-slate-500" /> Silver Tier
          </div>
        ...
        <div className="bg-yellow-50 border border-yellow-300 rounded-2xl p-4">
          <div className="text-xs font-bold uppercase text-yellow-800 flex items-center gap-1">
            <Award className="w-4 h-4 text-yellow-600" /> Gold Tier
          </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-3 rounded-2xl border border-gray-300 text-sm focus:outline-none focus:border-orange-500 bg-white"
        />
      </div>
                ...
                <tr key={ngo.id} className="hover:bg-orange-50/30 transition-colors">
                  ...
                  <td className="py-4 px-4">
                    <span className={`inline-flex items-center gap-1 font-bold ${
                      ...
                      {ngo.status === 'Verified' && <CheckCircle2 className="w-3.5 h-3.5" />}
                      {ngo.status === 'Rejected' && <XCircle className="w-3.5 h-3.5" />}
                      {ngo.status}
                    </span>
                  </td>
                    ...
                    {ngo.status !== 'Verified' ? (
                      <>
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold transition-all"
                        >
                          Approve (Silver)
                        </button>
                        <button
                          onClick={() => handleApprove(ngo.id, 'Gold')}
                          className="px-3 py-1.5 rounded-lg bg-yellow-500 hover:bg-yellow-600 text-white font-bold transition-all"
                        >
                          Elevate (Gold)
                        </button>
                        <button
                          onClick={() => handleReject(ngo.id)}
                          className="px-2.5 py-1.5 rounded-lg bg-red-100 hover:bg-red-200 text-red-700 font-bold transition-all"
                        >
                          Reject
                        </button>
                      </>
